...
func TestSuccessfulCloseMongoSession(t *testing.T) {
	_, err := setupSession()
	if err != nil {
		log.Error(context.Background(), "mongo instance not available, skip close tests", err)
		return
	}
	...
	if err = setUpTestData(session.Copy()); err != nil {
		log.Fatal(context.Background(), "failed to insert test data, skipping tests", err)
		os.Exit(1)
	}
